...
def split_grafx_params(
    ...
    """Split grafx-prune's flat per-processor params + drywet logits into strip + group.

    For each processor type, `construct_mixing_console` adds N_stems nodes
    first (one per stem, in stem order) and then N_groups nodes (one per
    group bus). So both `graph_parameters[proc][param]` (shape
    `(N_stems + N_groups, *param_shape)`) and the per-node entries of
    `solver.soft_mask` filtered to that processor type are sliceable as
    `[:n_stems]` (strip) and `[n_stems:]` (group).

    Drywet handling:
      - grafx-prune renders with `wet = sigmoid(soft_mask)`. We save the
        raw logit (`soft_mask` values) as `drywet_logit: (N, 1)` so the
        student's L_param trains in well-behaved logit space — see
        `models.grafx_console._DryWetLogitWrap` for the matching render
        contract. The shape-(1,) trailing dim matches the schema
        convention used for the inner processors' scalar params (e.g.
        `compressor.log_threshold`).
    """
    strip: dict[str, dict[str, torch.Tensor]] = {}
    group: dict[str, dict[str, torch.Tensor]] = {}
    soft_mask = soft_mask.detach().cpu()
    node_types_cpu = node_types.detach().cpu()
    ...
    for proc_name in processors:
        ...
        proc_idx = node_configs.node_type_to_index[proc_name]
        proc_softmask = soft_mask[node_types_cpu == proc_idx]
        assert proc_softmask.shape[0] == n_stems + n_groups, (
            f"{proc_name}: expected {n_stems + n_groups} soft_mask entries, "
            f"got {proc_softmask.shape[0]}"
        )
        strip[proc_name]["drywet_logit"] = proc_softmask[:n_stems].clone().unsqueeze(-1)
        group[proc_name]["drywet_logit"] = proc_softmask[n_stems:].clone().unsqueeze(-1)
    return strip, group
...
def run_labeler(
    ...
    # Extract optimized params + split strip vs group. soft_mask captures
    # the per-node drywet logit which Adam trains alongside the inner
    # processor params even in `mixing_console_full` (prune=False).
    strip_params, group_params = split_grafx_params(
        solver.graph_parameters,
        solver.soft_mask,
        solver.G_tensor.node_types,
        solver.node_configs,
        args.processors, n_stems, n_groups,
    )
...
def main() -> int:
    ...
    ap.add_argument("--total-epochs", type=int, default=12,
                    help="Training epochs. Default 12 (half of the paper's "
                         "24-epoch recipe) — matches `prune_start_epoch` in "
                         "grafx-prune's base.yaml, so it's the natural "
                         "stopping point for the no-pruning case. Pre-2026-05 "
                         "runs used 6 (smoke-test); those labels are "
                         "demonstrably undertrained (see notes/RUNS.md).")
    ap.add_argument("--steps-per-epoch", type=int, default=500,
                    help="Default 500 (paper recipe). Pre-2026-05 runs "
                         "used 100 for smoke-testing.")
