...
Grafx-prune expects each song under one base dir with this layout:

    <staging>/<session>/
      ├── correspondence.yaml   (group → list of stem filenames; from Diff-MST)
      ├── alignment.pickle      (mix↔rough_mix offset; from Diff-MST)
      ├── mix.wav               (engineer reference mix, stereo)
      └── stems/                (per-track stems, stereo)
          ├── 01_Kick.wav
          ├── …
...
This script:
    ...
    2. Resamples the source ref-mix wav from `source_audio/wavs/`
       (mapped via `cambridge_preview_destination_map.tsv`) to --target-sr stereo.
    3. Resamples every stem listed in correspondence.yaml from
       `source_audio/cambridge-mt/<session>_Full/<session>_Full/` to --target-sr.
...
def resample_to(src: Path, dst: Path, target_sr: int) -> None:
    ...
    if data.shape[1] == 1:
        ...
        data = data[:, :2]
    if sr != target_sr:
        data = soxr.resample(data, sr, target_sr, quality="VHQ").astype(np.float32, copy=False)
    sf.write(str(dst), data, target_sr)


def stage_session(session: str, staging_dir: Path, refwav_map: dict[str, Path], target_sr: int) -> dict:
...
def main() -> int:
    ...
    ap.add_argument("--target-sr", type=int, default=48_000,
                    help="Target sample rate for resampled stems + mix.")
    ...
    logging.info(f"staging {len(targets)} session(s) to {staging} "
                 f"(workers={args.workers}, target_sr={args.target_sr})")

    results: list[dict] = []
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        futures = {ex.submit(stage_session, s, staging, refwav_map, args.target_sr): s for s in targets}
